...
import { overrideSystemHandling, checkHeldKeysRecursive, uniq_fast } from './utils';

const BLACKLISTED_DOM_TARGETS = ['text'];
...
const useKeyboardShortcut = (shortcutKeys, callback, userOptions) => {
	...
	if (!shortcutKeys.length) throw new Error('The first parameter to `useKeyboardShortcut` must contain atleast one `KeyboardEvent.key` string.');
	...
	const keydownListener = useCallback(
		(keydownEvent) => {
			...
			if (options.ignoreInputFields && BLACKLISTED_DOM_TARGETS.indexOf(keydownEvent.target.type) >= 0) {
				return;
			}
			...
			const isHeldKeyCombinationValid = checkHeldKeysRecursive(loweredKey, null, shortcutArray, heldKeys.current);
		...
		// eslint-disable-next-line react-hooks/exhaustive-deps
		[shortcutKeysId, callback, options.overrideSystem, options.ignoreInputFields]
	);
