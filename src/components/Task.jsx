...
const Task = ({ task, variants, className }) => {
	const { setId, setUpdateTaskVisible, user, setSelectedTask, selectedTask } = useContext(FirebaseContext);
		...
		>
			<div className="fr justify-center items-center gap-3">
				...
				<div className="fc items-start justify-start">
					<p className={`transition-all sm:text-lg ${status === 'complete' ? 'line-through text-slate-500' : ''}`}>{text}</p>
